...
const MarketplaceSectionComponent = () => {
  ...
  const features = [
    { icon: BadgeCheck, title: t("marketplace.verificationAvailable"), description: t("marketplace.verificationAvailableDesc") },
    { icon: TrendingUp, title: t("marketplace.fairPrices"), description: t("marketplace.fairPricesDesc") },
    { icon: Users, title: t("marketplace.collectorCommunity"), description: t("marketplace.collectorCommunityDesc") },
  ];
        ...
        >
          <div className="text-center mb-14">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/5 border border-primary/20 backdrop-blur-sm mb-6">
              ...
              <span className="text-sm font-medium text-primary tracking-wide">{t("marketplace.badge")}</span>
            </div>
            <h2 className="font-display text-3xl md:text-4xl font-bold mb-5 tracking-tight">
              {t("marketplace.titlePlain")}
            </h2>
            <p className="text-muted-foreground text-base max-w-2xl mx-auto">{t("marketplace.subtitle")}</p>
          </div>
          ...
          >
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
              ...
              <Button asChild size="lg" variant="outline" className="group border-primary/20 hover:border-primary/40">
                <Link to="/full">
                  {t("marketplace.meetFull")}
                  <ArrowRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
                </Link>
              </Button>
            </div>
            <p className="text-xs text-muted-foreground mt-4">{t("marketplace.openToAll")}</p>
          </motion.div>
        </motion.div>
      </div>
