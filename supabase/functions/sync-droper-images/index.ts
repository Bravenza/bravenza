...
// Salva em: product-images/products/{sku}/0.webp ... 5.webp
...
async function updateSneakerModel(
  ...
  const { data, error } = await supabase
    .from("sneaker_models")
    .update({
      placeholder_image_url: imageUrls[0],      // imagem principal (índice 0)
      source_primary:        droperSourceUrl,    // URL da página do produto na droper
      source_secondary:      imageUrls.slice(1), // demais imagens (índices 1–5)
      image_status:          "synced",           // marca como sincronizado
      needs_official_image:  false,              // não precisa mais de imagem manual
    })
    .eq("sku", sku)
    .select("id");
