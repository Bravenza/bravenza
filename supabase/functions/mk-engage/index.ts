...
if(mt==="POST"&&a==="product-comment"){const b=await req.json();if(!b.product_id||!b.content)throw new Error("product_id e content obrigatórios");const{data:mem}=await sb.from("vault_members").select("client_name").eq("client_cpf",cpf).maybeSingle();const{data:comment,error}=await sb.from("marketplace_product_comments").insert({product_id:b.product_id,user_cpf:cpf,user_name:mem?.client_name||"Usuário",content:b.content,parent_id:b.parent_id||null,is_seller_reply:false}).select().single();if(error)throw error;return j({comment});}
if(mt==="GET"&&a==="product-reviews"){const pid=url.searchParams.get("product_id");if(!pid)throw new Error("product_id obrigatório");const{data:reviews}=await sb.from("marketplace_product_reviews").select("*").eq("product_id",pid).eq("is_visible",true).order("created_at",{ascending:false});const list=reviews||[];const avg=list.length>0?list.reduce((s:number,r:any)=>s+r.rating,0)/list.length:0;return j({reviews:list,average:Math.round(avg*10)/10,total:list.length});}
...
}catch(e:any){console.error("mk-engage error:",e);return j({error:e.message},500);}
});
