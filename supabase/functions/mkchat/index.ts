// Marketplace Chat Edge Function
...
serve(async (req) => {
  ...
  try {
    ...
    if (mt === "POST" && a === "send-message") {
      ...
      const { data: mg, error } = await sb.from("vault_marketplace_messages").insert({ order_id: b.order_id || null, listing_id: b.listing_id || null, sender_cpf: cpf, sender_name: b.sender_name, message: b.message, is_admin: b.is_admin || false }).select().single();
      if (error) throw error;
