...
if(mt==="PUT"&&a==="update-order-status"){
  ...
  if(b.status==="shipped"){u.shipped_at=new Date().toISOString();u.tracking_code=b.tracking_code||null;}
    ...
    await requireAdminByToken(sb,ah!);
...
if(mt==="GET"&&a==="admin-orders"){await requireAdminByToken(sb,ah!);const st=url.searchParams.get("status");const search=url.searchParams.get("search");const dateFrom=url.searchParams.get("date_from");const dateTo=url.searchParams.get("date_to");const pg=Math.max(1,parseInt(url.searchParams.get("page")||"1"));const ps=Math.min(100,Math.max(1,parseInt(url.searchParams.get("pageSize")||"20")));const fr=(pg-1)*ps;const to=fr+ps-1;let q=sb.from("vault_marketplace_orders").select(`*,listing:vault_marketplace_listings(title,brand,model,size,photos,condition)`,{count:"exact"}).order("created_at",{ascending:false});if(st&&st!=="all")q=q.eq("status",st);if(search){q=q.or(`order_code.ilike.%${search}%,buyer_name.ilike.%${search}%`);}if(dateFrom)q=q.gte("created_at",dateFrom);if(dateTo)q=q.lte("created_at",dateTo);q=q.range(fr,to);const{data,error,count}=await q;if(error)throw error;return j({orders:data||[],total:count||0,page:pg,pageSize:ps});}
if(mt==="GET"&&a==="admin-disputes"){await requireAdminByToken(sb,ah!);const disputeStatus=url.searchParams.get("status");const openedBefore=url.searchParams.get("opened_before");const pg=Math.max(1,parseInt(url.searchParams.get("page")||"1"));const ps=Math.min(100,Math.max(1,parseInt(url.searchParams.get("pageSize")||"50")));const fr=(pg-1)*ps;const to=fr+ps-1;let q=sb.from("vault_marketplace_orders").select(`*,listing:vault_marketplace_listings(title,brand,model,size,photos,condition)`,{count:"exact"}).not("dispute_status","is",null).order("dispute_opened_at",{ascending:false});if(disputeStatus==="open")q=q.eq("dispute_status","open");else if(disputeStatus==="resolved")q=q.eq("dispute_status","resolved");if(openedBefore)q=q.lt("dispute_opened_at",openedBefore);q=q.range(fr,to);const{data,error,count}=await q;if(error)throw error;return j({disputes:data||[],total:count||0,page:pg,pageSize:ps});}
