...
if(mt==="GET"&&a==="admin-orders"){await requireAdmin(sb,ah!);const st=url.searchParams.get("status");const pg=Math.max(1,parseInt(url.searchParams.get("page")||"1"));const ps=Math.min(100,Math.max(1,parseInt(url.searchParams.get("pageSize")||"50")));const fr=(pg-1)*ps;const to=fr+ps-1;let q=sb.from("vault_marketplace_orders").select(`*,listing:vault_marketplace_listings(title,brand,model,size,photos,condition)`,{count:"exact"}).order("created_at",{ascending:false});if(st&&st!=="all")q=q.eq("status",st);q=q.range(fr,to);const{data,error,count}=await q;if(error)throw error;return j({orders:data||[],total:count||0,page:pg,pageSize:ps});}
if(mt==="GET"&&a==="admin-disputes"){const pg=Math.max(1,parseInt(url.searchParams.get("page")||"1"));const ps=Math.min(100,Math.max(1,parseInt(url.searchParams.get("pageSize")||"50")));const fr=(pg-1)*ps;const to=fr+ps-1;const{data,error,count}=await sb.from("vault_marketplace_orders").select(`*,listing:vault_marketplace_listings(title,brand,model,size,photos,condition)`,{count:"exact"}).not("dispute_status","is",null).order("dispute_opened_at",{ascending:false}).range(fr,to);if(error)throw error;return j({disputes:data||[],total:count||0,page:pg,pageSize:ps});}
