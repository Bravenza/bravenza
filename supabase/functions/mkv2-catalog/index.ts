...
const makeSB = () => createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!);
...
async function cpfFromReq(req: Request, sb: any): Promise<string> {
  const ah = req.headers.get("authorization");
  if (!ah || !ah.startsWith("Bearer ")) return "visitor";
  try {
    const { data: u } = await sb.auth.getUser(ah.replace("Bearer ", ""));
    if (!u || !u.user) return "visitor";
    const { data: p } = await sb.from("client_profiles").select("cpf").eq("user_id", u.user.id).single();
    return (p && p.cpf) ? p.cpf : "visitor";
  } catch {
    return "visitor";
  }
}
...
Deno.serve(async (req) => {
  ...
  const mt = req.method;
  const cpf = await cpfFromReq(req, sb);
  ...
  try {
    if (mt === "GET" && action === "catalog-products") {
      ...
      const nc = hasMore ? page[page.length - 1].created_at : null;
      return json({ products: page, total: count || 0, next_cursor: nc, has_more: hasMore });
    }
