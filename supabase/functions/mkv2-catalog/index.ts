...
if(mt==="GET"&&a==="catalog-product"){
  ...
  const offers=od||[];const sizes=[...new Set(offers.map((o:any)=>o.size))].sort();
  // Calculate max interest_free_installments across all offers for display
  const maxInterestFree=offers.reduce((m:number,o:any)=>Math.max(m,o.interest_free_installments||0),0);
  return j({product:data,offers,sizes,max_interest_free_installments:maxInterestFree});
}
