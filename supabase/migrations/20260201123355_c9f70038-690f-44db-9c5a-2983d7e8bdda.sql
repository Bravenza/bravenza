...
-- Drop all overloaded versions of get_vault_member
DROP FUNCTION IF EXISTS get_vault_member(text);
DROP FUNCTION IF EXISTS get_vault_member(varchar);
DROP FUNCTION IF EXISTS get_vault_member(character varying);

-- Recreate with VARCHAR parameter only
CREATE OR REPLACE FUNCTION get_vault_member(p_cpf VARCHAR)
RETURNS TABLE(
  id UUID,
  tier vault_tier,
