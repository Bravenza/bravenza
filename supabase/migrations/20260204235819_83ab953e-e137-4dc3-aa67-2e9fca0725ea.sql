-- Fix get_vault_community_feed to include has_liked and author_items_count
...
BEGIN
  ...
  SELECT 
    ...
    (SELECT COUNT(*)::INTEGER FROM vault_items vi WHERE vi.user_id = vm.id) as author_items_count,
    p.type::TEXT, 
    p.title::TEXT,
    p.content::TEXT,
    p.attachments, 
...
-- Fix get_following_feed to include has_liked and author_items_count
